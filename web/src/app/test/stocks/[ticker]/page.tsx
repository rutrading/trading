...
const STOCKS: Record<string, { name: string; price: number; change: number; open: number; high: number; low: number; prevClose: number; volume: string; marketCap: string; pe: number; week52High: number; week52Low: number; avgVolume: string }> = {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${stock?.name ?? upper} (${upper}) - R U Trading` };
}
...
export default async function StockPage({ params }: Props) {
  ...
  const upper = ticker.toUpperCase();
  const stock = STOCKS[upper] ?? {
    name: upper, price: 0, change: 0, open: 0, high: 0, low: 0, prevClose: 0,
    volume: "—", marketCap: "—", pe: 0, week52High: 0, week52Low: 0, avgVolume: "—",
  };

  return <StockDetail ticker={upper} stock={stock} />;
}
